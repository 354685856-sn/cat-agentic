...
export function App() {
  ...
  const [accountStatus, setAccountStatus] = useState<'unknown' | 'signed-in' | 'signed-out'>('unknown')
  ...
  useEffect(() => {
    if (!nativeShell || !workspacePath) return
    void invoke<WorkspaceSnapshot>('workspace_snapshot', { path: workspacePath }).then(setWorkspaceSnapshot).catch(() => setWorkspaceSnapshot(null))
  }, [nativeShell, workspacePath])
  useEffect(() => {
  ...
  async function submit() {
    ...
    try {
      ...
      if (accountStatus === 'unknown') { const account = await client.readAccount(); setAccountStatus(account.account ? 'signed-in' : 'signed-out') }
  ...
  async function signInWithChatGPT() {
    if (!nativeShell) { setNotice(textFor('登录需要桌面应用。', 'Sign-in requires the desktop app.')); return }
    try {
      if (!initialized.current) {
        await client.initialize({ name: 'cat-codex', title: 'Cat Codex', version: '0.1.0' }, workspacePath ? { cwd: workspacePath } : undefined)
        initialized.current = true
      }
      const result = await client.startAccountLogin('chatgpt')
      if (result.authUrl) window.open(result.authUrl, '_blank', 'noopener,noreferrer')
      setNotice(textFor('已打开 ChatGPT 登录页面；完成登录后返回 Cat Codex。', 'ChatGPT sign-in opened in your browser. Return to Cat Codex when complete.'))
    } catch (error) { setNotice(`${textFor('登录启动失败：', 'Unable to start sign-in: ')}${error instanceof Error ? error.message : String(error)}`) }
  }
  ...
  async function inspectWorkspaceFile(relative: string) {
    setInspectorData((current) => ({ ...current, selectedFile: relative, fileContent: '', fileError: undefined }))
    if (!nativeShell || !workspacePath) return
    try {
      const file = await invoke<{ path: string; content: string; truncated: boolean }>('read_workspace_file', { workspace: workspacePath, relative })
      setInspectorData((current) => ({ ...current, selectedFile: file.path, fileContent: `${file.content}${file.truncated ? '\n\n[File preview truncated at 512 KiB]' : ''}`, fileError: undefined }))
    } catch (error) {
      setInspectorData((current) => ({ ...current, fileError: error instanceof Error ? error.message : String(error) }))
    }
  }
  ...
  return (
    <div className={`app-shell ${theme === 'light' ? 'light-theme' : ''}`}>
      ...
      {activeView === 'home' ? <section className={`chat-home-shell ${homeInspectorOpen ? 'with-inspector' : ''}`} aria-label={textFor('Cat Codex 主页', 'Cat Codex home')}>
        ...
        {homeInspectorOpen && <HomeInspector activeTab={activeTab} setActiveTab={setActiveTab} language={language} data={inspectorData} onSelectFile={inspectWorkspaceFile} onClose={() => setHomeInspectorOpen(false)} />}
        ...
        <aside className="sidebar">
          ...
          <div className="project-row active"><span className="project-glyph">N</span><span className="project-copy"><strong>{workspaceSnapshot?.root.split(/[\\/]/).pop() || textFor('新建项目', 'New project')}</strong><small>{workspacePath || '~/Documents/Codex'}</small></span><span className="project-count">{workspaceSnapshot?.fileCount ?? 0}</span></div>
          ...
          <div className="sidebar-bottom"><button className="sidebar-link" onClick={() => setActiveTab('plugins')}><Icon name="spark" /> {textFor('插件注册表', 'Plugin registry')} <span className="sidebar-pill">0</span></button><button className="sidebar-link" onClick={() => { if (workspacePath) void invoke('open_workspace', { path: workspacePath }); else void chooseWorkspace() }}><Icon name="folder" /> {textFor('打开文件夹', 'Open folder')}</button><button className="sidebar-link" onClick={() => openSettings()}><Icon name="settings" /> {textFor('偏好设置', 'Preferences')} <kbd>⌘ ,</kbd></button></div>
        </aside>
        ...
        <main className="conversation">
          ...
          <div className="conversation-scroll">
            ...
            <section className="connection-note"><div className="note-icon"><Icon name="lock" /></div><div><strong>{accountStatus === 'signed-in' ? textFor('ChatGPT 账户已连接', 'ChatGPT account connected') : textFor('App Server 需要登录', 'App Server sign-in required')}</strong><p>{textFor('会话通过本机 Codex App Server 运行；登录凭据由官方运行时管理。', 'Sessions run through the local Codex App Server; credentials are managed by the official runtime.')}</p></div><button onClick={signInWithChatGPT}>{accountStatus === 'signed-in' ? textFor('重新登录', 'Sign in again') : textFor('登录 ChatGPT', 'Sign in with ChatGPT')}</button></section>
          </div>
        ...
        <aside className="inspector">
          ...
          {activeTab === 'files' && <div className="inspector-content"><div className="tree-root"><Icon name="folder" /><strong>{workspaceSnapshot?.root.split(/[\\/]/).pop() || textFor('选择工作区', 'Choose workspace')}</strong><span>{workspaceSnapshot ? textFor(`${workspaceSnapshot.fileCount} 个文件`, `${workspaceSnapshot.fileCount} files`) : textFor('尚未索引', 'Not indexed')}</span></div><div className="file-tree">{(workspaceSnapshot?.files.slice(0, 80) || []).map((file) => <button className="tree-row" key={file} onClick={() => { void inspectWorkspaceFile(file); setActiveTab('diff') }}><Icon name="file" /><span>{file}</span></button>)}</div><div className="inspector-section"><div className="inspector-label">{textFor('工作区', 'WORKSPACE')}</div><div className="stat-line"><span>{textFor('分支', 'Branch')}</span><strong><Icon name="git" /> {workspaceSnapshot?.branch || '—'}</strong></div><div className="stat-line"><span>{textFor('目录', 'Directories')}</span><strong>{workspaceSnapshot?.directoryCount ?? 0}</strong></div></div></div>}
          {activeTab === 'diff' && <div className="inspector-content"><div className="diff-summary"><span className="diff-add">+ 318</span><span className="diff-remove">− 0</span><span className="diff-files">{inspectorData.selectedFile || textFor('4 个文件', '4 files')}</span></div><pre className="diff-preview"><code>{inspectorData.diff || inspectorData.fileContent || <><span className="code-muted">apps/cat-codex/</span>{'\n'}<span className="code-add">+ src/App.tsx</span>{'\n'}<span className="code-add">+ src/styles.css</span>{'\n'}<span className="code-add">+ src/lib/codex/client.ts</span>{'\n'}<span className="code-add">+ src/lib/providers.ts</span></>}</code></pre>{inspectorData.fileError && <p className="empty-copy">{inspectorData.fileError}</p>}<p className="empty-copy">{textFor('更改会保留在本地，直到你审查并应用。', 'Changes stay local until you review and apply them.')}</p></div>}
